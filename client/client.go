...
type Client interface {
	...
	GetSelectResults(ops []ovsdb.Operation, results []ovsdb.OperationResult, target interface{}, index int) error
}
...
//
// Parameters:
//   - ops: The operations that were sent to Transact (must match results length)
//   - results: The operation results returned from Transact
//   - target: A pointer to a slice of models where results will be stored (e.g., *[]*MyModel)
//   - index: Which select query to retrieve (0-based index)
//
// Select Queries:
// When you have multiple select queries for the same table (created by separate calls
// to Select()), they are grouped by the select query they belong to. The index parameter
// specifies which query's results to retrieve:
//   - index = 0: First select query (most common case)
//   - index = 1: Second select query, etc.
//
// For operations generated by WhereAny() or WhereCache(), all operations belong to
// the same select query, so index should be 0.
//
// The function automatically:
//   - Filters operations to match the target table type
//   - Groups results by select query
//   - Merges and deduplicates results within the selected query
//   - Populates the target slice with the final results
//
// Returns an error if:
//   - ops and results length don't match
//   - target is not a pointer to a slice
//   - index is out of range for available select queries
//   - any operation result contains an error
func (o *ovsdbClient) GetSelectResults(ops []ovsdb.Operation, results []ovsdb.OperationResult, target interface{}, index int) error {
	...
	for _, result := range selectedResults {
		...
		for _, rowData := range result.Rows {
			...
			if err := db.model.Mapper.GetRowDataWithUUID(&rowData, info); err != nil {
